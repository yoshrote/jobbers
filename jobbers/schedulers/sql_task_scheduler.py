"""
SQLTaskScheduler — SQLAlchemy-backed implementation of AtomicTaskSchedulerProtocol.

Uses the ``task_schedule`` table defined in ``jobbers.migrations.schema``.
"""
...
class SQLTaskScheduler:
        ...
        async with self._sf() as session:
            async with session.begin():
                await session.execute(
                    delete(task_schedule).where(task_schedule.c.task_id == str(task_id))
                )
    ...
    async def get_run_at(self, task_id: ULID) -> dt.datetime | None:
        ...
        async with self._sf() as session:
            result = await session.execute(
                select(task_schedule.c.run_at).where(
                    task_schedule.c.task_id == str(task_id)
                )
            )
            row = result.first()
            return _ensure_utc_nn(row.run_at) if row is not None else None
    ...
    async def next_due_bulk(
        ...
        async with self._sf() as session:
            async with session.begin():
                ...
                await session.execute(
                    delete(task_schedule).where(task_schedule.c.task_id.in_(task_ids))
                )
        return [(_row_to_task(row), _ensure_utc_nn(row.run_at)) for row in rows]
    ...
    async def get_by_filter(
        ...
        for row in rows:
            ...
            if task_version is not None and task.version != task_version:
                continue
            results.append((task, _ensure_utc_nn(row.run_at)))
        return results
