...
1. ``true`` / ``false`` (case-insensitive) → :class:`bool`
2. Decimal integer literal → :class:`int`
3. Decimal float literal → :class:`float`
4. Quoted string (``"..."`` or ``'...'``) → :class:`str` (quotes stripped)
5. Anything else → :class:`str`
...
# Parses: task_name[:queue][(params)]
_LABEL_RE = re.compile(
    r"^(?P<name>[a-zA-Z_][a-zA-Z0-9_]*)(?::(?P<queue>[a-zA-Z0-9_-]+))?(?:\((?P<params>[^)]*)\))?$"
)
...
def _parse_param_value(raw: str) -> int | float | bool | str:
    """Coerce a raw parameter value token to a Python scalar."""
    s = raw.strip()
    lower = s.lower()
    if lower == "true":
        return True
...
def _parse_label(label: str) -> tuple[str, str, dict[str, Any]]:
    """
    Parse a node label into ``(task_name, queue, parameters)``.
...
def parse_mermaid_dag(text: str) -> list[DAGNode]:
    ...
    # Parse each label → (task_name, queue, parameters).
    ...
    dag_nodes: dict[str, DAGNode] = {
        nid: DAGNode(name, queue=queue, parameters=params, task_id=ULID())
        for nid, (name, queue, params) in parsed.items()
    }
