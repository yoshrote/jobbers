...
- **`StateManager`/`TaskProcessor` changes touching atomicity, ordering, idempotency, or race-prone paths need a real-backend test, not just a `DummyTaskAdapter` one.** `DummyTaskAdapter`/`DummyTaskSubmit` are deliberately simplified stubs — they don't replicate Lua-script atomicity, sliding-window rate-limit pruning, row-lock races, or NX/dispatch-lock semantics. A `Dummy*`-based orchestration test only proves StateManager called the right methods in the right order; it cannot prove the interaction is actually atomic/race-free against a real backend. Multiple past bugs (rate-limiter races, atomic-dispatch races, saga-mode submit paths that saved a task blob but never enqueued it) shipped without any test catching them because only `Dummy*`-backed coverage existed. When touching submit/retry/rate-limit/fan-in/dispatch-lock logic (or anything else with real atomicity or ordering guarantees), add at least one test against a real-ish backend — the `state_manager_real_ta` fixture (`RedisTaskState`/`RedisTaskSubmit` via FakeRedis, real Lua scripts) or the `task_adapter`/`scheduler`/`cron_dag_scheduler` fixtures — in addition to whatever `Dummy*`-based orchestration test already exists. Don't try to make `Dummy*` itself protocol-compliant to close this gap — that turns it into a second adapter to maintain; instead route the specific at-risk interaction to a real-backend test.
- **Adapter fixtures**
  ...
  - `state_manager_real_ta` (in `tests/conftest.py`) — `StateManager` backed by real `RedisTaskState`/`RedisTaskSubmit` via FakeRedis (real Lua scripts, real sorted-set/rate-limit semantics) instead of `DummyTaskAdapter`; use this for orchestration tests that need to prove an interaction is actually atomic/race-free, not just that the right methods were called.
...
- `jobbers/task_processor.py` line 182 — `hints = {}` fallback when `get_type_hints()` raises; only reachable with unresolvable forward references in task annotations
