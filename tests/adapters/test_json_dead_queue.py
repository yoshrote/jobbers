"""Tests for JsonDeadQueue (Redis Stack: RedisJSON + RediSearch)."""
...
@pytest.mark.asyncio
async def test_add_and_get_by_id(dq, task_adapter):
    task = make_task()
...
@pytest.mark.asyncio
async def test_get_by_ids_multiple(dq, task_adapter):
...
@pytest.mark.asyncio
async def test_get_by_ids_returns_only_matching(dq, task_adapter):
    t1 = make_task(task_id="01JQC31AJP7TSA9X8AEP64XG01")
    t2 = make_task(task_id="01JQC31AJP7TSA9X8AEP64XG02")
    for t in (t1, t2):
        await task_adapter.save_task(t)
...
@pytest.mark.asyncio
async def test_get_by_ids_empty_list_returns_empty(dq, task_adapter):
    task = make_task()
    await task_adapter.save_task(task)
    await dq.add(task, FAILED_AT)
...
@pytest.mark.asyncio
async def test_get_by_filter_queue(dq, task_adapter):
...
@pytest.mark.asyncio
async def test_get_by_filter_task_name(dq, task_adapter):
...
@pytest.mark.asyncio
async def test_get_by_filter_task_version(dq, task_adapter):
...
@pytest.mark.asyncio
async def test_get_by_filter_version_zero(dq, task_adapter):
...
@pytest.mark.asyncio
async def test_get_by_filter_combined(dq, task_adapter):
...
@pytest.mark.asyncio
async def test_get_by_filter_no_criteria_returns_all(dq, task_adapter):
    t1 = make_task(task_id="01JQC31AJP7TSA9X8AEP64XG01")
    t2 = make_task(task_id="01JQC31AJP7TSA9X8AEP64XG02")
    for t in (t1, t2):
        await task_adapter.save_task(t)
...
@pytest.mark.asyncio
async def test_get_by_filter_limit_respected(dq, task_adapter):
    tasks = [
...
@pytest.mark.asyncio
async def test_get_by_filter_no_match_returns_empty(dq, task_adapter):
...
async def test_get_by_filter_sorted_by_failed_at_desc(dq, task_adapter):
    ...
    await dq.add(t1, EARLIER)
    await dq.add(t2, LATER)
...
@pytest.mark.asyncio
async def test_remove_deletes_entry(dq, task_adapter):
    task = make_task()
    await task_adapter.save_task(task)
    await dq.add(task, FAILED_AT)
...
@pytest.mark.asyncio
async def test_remove_leaves_other_entries_intact(dq, task_adapter):
    t1 = make_task(task_id="01JQC31AJP7TSA9X8AEP64XG01")
    t2 = make_task(task_id="01JQC31AJP7TSA9X8AEP64XG02")
    for t in (t1, t2):
        await task_adapter.save_task(t)
...
@pytest.mark.asyncio
async def test_remove_many_deletes_all(dq, task_adapter):
    t1 = make_task(task_id="01JQC31AJP7TSA9X8AEP64XG01")
    t2 = make_task(task_id="01JQC31AJP7TSA9X8AEP64XG02")
    for t in (t1, t2):
        await task_adapter.save_task(t)
...
@pytest.mark.asyncio
async def test_remove_many_leaves_unmentioned_entries_intact(dq, task_adapter):
    t1 = make_task(task_id="01JQC31AJP7TSA9X8AEP64XG01")
    t2 = make_task(task_id="01JQC31AJP7TSA9X8AEP64XG02")
...
@pytest.mark.asyncio
async def test_remove_many_empty_list_is_silent(dq, task_adapter):
    task = make_task()
    await task_adapter.save_task(task)
    await dq.add(task, FAILED_AT)
...
@pytest.mark.asyncio
async def test_remove_many_partial_match(dq, task_adapter):
    t1 = make_task(task_id="01JQC31AJP7TSA9X8AEP64XG01")
    await task_adapter.save_task(t1)
...
@pytest.mark.asyncio
async def test_get_history_returns_errors_from_task_blob(dq, task_adapter):
...
@pytest.mark.asyncio
async def test_get_history_empty_when_no_errors(dq, task_adapter):
...
@pytest.mark.asyncio
async def test_get_history_isolated_by_task_id(dq, task_adapter):
...
@pytest.mark.asyncio
async def test_clean_removes_old_entries(dq, task_adapter):
