...
@pytest.mark.asyncio
async def test_stage_requeue_adds_task_to_queue(adapter, redis):
    ...
    members = await redis.zrange(adapter.TASKS_BY_QUEUE(queue="default"), 0, -1)
    assert bytes(ULID1) in members


@pytest.mark.asyncio
async def test_stage_requeue_saves_task_data(adapter, redis):
...
@pytest.mark.asyncio
async def test_stage_requeue_uses_submitted_at_as_score(adapter, redis):
    ...
    score = await redis.zscore(adapter.TASKS_BY_QUEUE(queue="default"), bytes(ULID1))
    assert score == pytest.approx(FROZEN_TIME.timestamp())


@pytest.mark.asyncio
async def test_stage_requeue_does_not_duplicate_queue_entry(adapter, redis):
...
@pytest.mark.asyncio
async def test_remove_task_heartbeat_removes_entry(adapter, redis):
    ...
    assert await redis.zscore(adapter.HEARTBEAT_SCORES(queue="default"), bytes(ULID1)) is not None

    await adapter.remove_task_heartbeat(task)
    assert await redis.zscore(adapter.HEARTBEAT_SCORES(queue="default"), bytes(ULID1)) is None
...
@pytest.mark.asyncio
async def test_get_active_tasks_returns_empty_for_no_heartbeats(adapter):
...
@pytest.mark.asyncio
async def test_get_active_tasks_returns_tasks_with_heartbeats(adapter, redis):
...
@pytest.mark.asyncio
async def test_clean_noop_when_no_age_params(adapter, redis):
...
@pytest.mark.asyncio
async def test_get_next_task_skips_missing_data_and_returns_none(adapter, redis):
    """When a task is popped from the queue but its blob is absent, it is logged and None is returned."""
    fake_id = ULID1
    queue_key = adapter.TASKS_BY_QUEUE(queue="default").encode() if isinstance(adapter.TASKS_BY_QUEUE(queue="default"), str) else adapter.TASKS_BY_QUEUE(queue="default")
    ...
    with patch.object(adapter.data_store, "bzpopmin", new_callable=AsyncMock, side_effect=pop_results):
        result = await adapter.get_next_task(queues={"default"}, pop_timeout=1)
    assert result is None
    # The missing ID should be tracked in the DLQ missing-data key
    assert await redis.zscore(adapter.DLQ_MISSING_DATA, bytes(fake_id)) is not None
