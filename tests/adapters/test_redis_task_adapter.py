"""
Redis-specific tests for RedisTaskState/RedisTaskSubmit and RedisJSONTaskState/RedisJSONTaskSubmit.

Uses the ``redis_task_adapter`` fixture (parametrized over ["redis", "redis_json"]), which yields
a (state, submit) tuple.  Tests the atomic pipeline path (data_store.pipeline(), stage_* methods) and
...
async def test_submit_task_adds_to_sorted_set(redis_task_adapter):
    ...
    task_list = await state.data_store.zrange(
        state.TASKS_BY_QUEUE(queue="default"), 0, -1
    )
...
async def test_submit_task_twice_does_not_duplicate_queue_entry(redis_task_adapter):
    ...
    task_list = await state.data_store.zrange(
        state.TASKS_BY_QUEUE(queue=task.queue), 0, -1
    )
...
async def test_stage_requeue_adds_task_to_queue(redis_task_adapter):
    ...
    members = await state.data_store.zrange(
        state.TASKS_BY_QUEUE(queue="default"), 0, -1
    )
...
async def test_stage_requeue_uses_submitted_at_as_score(redis_task_adapter):
    ...
    score = await state.data_store.zscore(
        state.TASKS_BY_QUEUE(queue="default"), bytes(ULID1)
    )
...
async def test_stage_remove_from_queue_removes_from_type_index(redis_task_adapter):
    ...
    state, submit = redis_task_adapter
    task = make_task()
    await state.data_store.sadd(
        state.TASK_BY_TYPE_IDX(name="my_task"), bytes(ULID1)
    )
    ...
    members = await state.data_store.smembers(
        state.TASK_BY_TYPE_IDX(name="my_task")
    )
...
async def test_remove_task_heartbeat_removes_from_sorted_set(redis_task_adapter):
    ...
    assert (
        await state.data_store.zscore(
            state.HEARTBEAT_SCORES(queue="default"), bytes(ULID1)
        )
        is not None
    )
...
async def test_update_task_heartbeat_adds_to_sorted_set(redis_task_adapter):
    ...
    scores = await state.data_store.zrange(
        state.HEARTBEAT_SCORES(queue=task.queue), 0, -1, withscores=True
    )
...
async def test_get_next_task_skips_missing_data_and_returns_none(redis_task_adapter):
    ...
    with patch.object(
        submit._data_store, "bzpopmin", new_callable=AsyncMock, side_effect=pop_results
    ):
        result = await submit.get_next_task(queues={"default"}, pop_timeout=1)
    assert result is None
    assert (
        await state.data_store.zscore(state.DLQ_MISSING_DATA, bytes(fake_id))
        is not None
    )
...
async def test_get_stale_tasks_filters_out_missing_blobs(redis_task_adapter):
    ...
    await state.data_store.delete(state.TASK_DETAILS(task_id=stale_task.id))

    stale_tasks = [
        task async for task in state.get_stale_tasks({"default"}, dt.timedelta(minutes=5))
    ]
...
async def test_clean_noop_when_no_age_params_leaves_sorted_set_intact(redis_task_adapter):
    ...
    now = dt.datetime.now(dt.UTC)
    await state.clean(queues={b"default"}, now=now)
...
async def test_clean_max_queue_age_removes_old_entries_from_sorted_set(redis_task_adapter):
    ...
    await submit.submit_task(old)
    await submit.submit_task(recent)

    cutoff = FROZEN_TIME - dt.timedelta(days=1)
    await state.clean(queues={b"default"}, now=FROZEN_TIME, max_queue_age=cutoff)
...
async def test_clean_min_queue_age_removes_entries_from_sorted_set(redis_task_adapter):
    ...
    await state.clean(queues={b"default"}, now=FROZEN_TIME, min_queue_age=floor)
...
async def test_clean_terminal_tasks_removes_heartbeat_entry(redis_task_adapter):
    ...
    assert (
        await state.data_store.zscore(
            state.HEARTBEAT_SCORES(queue="default"), ULID1.bytes
        )
        is None
    )
...
async def test_clean_terminal_tasks_removes_type_index_entry(redis_task_adapter):
    ...
    await state.save_task(task)
    await state.data_store.sadd(
        state.TASK_BY_TYPE_IDX(name="test_task"), ULID1.bytes
    )
    await state.clean_terminal_tasks(FROZEN_TIME, dt.timedelta(days=7))
    members = await state.data_store.smembers(
        state.TASK_BY_TYPE_IDX(name="test_task")
    )
...
async def test_clean_dag_runs_removes_stale_entries_from_sorted_set(redis_task_adapter):
    ...
    await state.data_store.zadd(state.DAG_RUNS, {bytes(dag_run_id): old_score})
...
async def test_clean_dag_runs_noop_when_nothing_stale(redis_task_adapter):
    ...
    await state.data_store.zadd(state.DAG_RUNS, {bytes(dag_run_id): recent_score})
...
async def test_stage_register_dag_run_noop_when_no_dag_run_id(redis_task_adapter):
    ...
    count = await state.data_store.zcard(state.DAG_RUNS)
    assert count == 0
...
async def test_submit_rate_limited_enqueues_when_empty(redis_task_adapter, redis_task_adapter_dt_module):
    ...
    assert bytes(ULID1) in await state.data_store.zrange(
        state.QUEUE_RATE_LIMITER(queue="default"), 0, -1
    )
    assert bytes(ULID1) in await state.data_store.zrange(
        state.TASKS_BY_QUEUE(queue="default"), 0, -1
    )
...
async def test_submit_rate_limited_enqueues_with_room(redis_task_adapter, redis_task_adapter_dt_module):
    ...
    with patch(redis_task_adapter_dt_module) as mock_dt:
        ...
        result = await submit.submit_rate_limited_task(task, _default_queue_config())
    assert result is True
    assert bytes(ULID2) in await state.data_store.zrange(
        state.TASKS_BY_QUEUE(queue="default"), 0, -1
    )
...
async def test_submit_rate_limited_prunes_expired_entries(redis_task_adapter, redis_task_adapter_dt_module):
    ...
    with patch(redis_task_adapter_dt_module) as mock_dt:
        ...
        result = await submit.submit_rate_limited_task(task, _default_queue_config())
    assert result is True
    assert new_id.bytes in await state.data_store.zrange(
        state.TASKS_BY_QUEUE(queue="default"), 0, -1
    )
...
async def test_submit_rate_limited_rejects_when_full(redis_task_adapter, redis_task_adapter_dt_module):
    ...
    assert ULID5.bytes not in await state.data_store.zrange(
        state.TASKS_BY_QUEUE(queue="default"), 0, -1
    )
...
async def test_submit_rate_limited_concurrent_respects_limit(redis_task_adapter):
    ...
    results = await asyncio.gather(
        *[submit.submit_rate_limited_task(t, queue_config) for t in tasks]
    )

    accepted = sum(1 for r in results if r)
    assert accepted == limit
    assert (
        await state.data_store.zcard(state.QUEUE_RATE_LIMITER(queue="default"))
        == limit
    )
    assert (
        await state.data_store.zcard(state.TASKS_BY_QUEUE(queue="default")) == limit
    )
